...
#include <limits>
#include <vector>
#include <format> //Similar to f-strings
...
class Character{
    public:
        ...
        std::string Move2;
        std::string Move3;
        std::string Movebuff1;
        std::string Movebuff2;
        std::string Movebuff3;
        int MoveEffect1;
        int MoveEffect2;
        int MoveEffect3;
        ...
        void MoveUse1(Character &e, Character &r){ //Mainly the attack towards someone
            std::cout << e.Name << " used " << e.Move1 << "! It did "<< ((float)e.ATK/100)/((float)r.DEF/100) << " DMG to " << r.Name << std::endl;
            r.HP -= ((float)e.ATK/100)/((float)r.DEF/100);
            
        }

        void MoveUse2(Character &e, Character &r){ 
            std::cout << e.Name << " used " << e.Move1 << "! It did "<< ((float)e.ATK/100)/((float)r.DEF/100) << " DMG! to " << r.Name << std::endl;
            r.HP -= ((float)e.ATK/100)/((float)r.DEF/100);
        ...
        void MoveUse3(Character &e, Character &r){ 
            std::cout << e.Name << " used " << e.Move1 << "! It did "<< ((float)e.ATK/100)/((float)r.DEF/100) << " DMG! to " << r.Name << std::endl;
            r.HP -= ((float)e.ATK/100)/((float)r.DEF/100);
        }
        ...
        bool Faint(){
            if (HP <= 0){
                std::cout<< Name << " has fainted! He cannot perform any actions!" << std::endl;
                return true;
            }else{
                return false;
            }
...
class Player: public Character{
    public:
        ...
        int MoveUse2(Player &e, Player &r){
            std::cout << e.Name << " used " << e.Move2 << " to buff " << r.Name << "'s " << e.Movebuff2 << " by " << e.MoveEffect2 << "!" << std::endl;
            if(e.Movebuff2 == "DEF"){
                r.DEF += e.MoveEffect2;
            }else if(e.Movebuff2 == "HP"){
                r.HP += e.MoveEffect2;
            }else if(e.Movebuff2 == "ATK"){
                r.ATK += e.MoveEffect2;
            }

        }

        std::string MoveUse3(Player &e, Player &r){ //FUSION HA
            std::cout << e.Name << " asked " << r.Name << " to fuse! They've successfully fused!" << std::endl;
            std::string New_name;
            if (r.Faint()){
                return "Your ally cannot fuse due to its 0 HP!";
            }else{
                for (int i = 0; i < e.Name.size()/2; i++){
                    New_name += e.Name[i];
                    
                }
                for (int i = r.Name.size()/2; i < r.Name.size(); i++){
                    New_name += r.Name[i];
                }
                return New_name;
            }
